            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              {activeTab === "general" && (
                <div className="space-y-4">
                  ...
                  {/* Mobile & Cloud Backend URL */}
                  <div className="pt-3 border-t border-white/5 flex flex-col gap-1.5 text-left">
                    <span className="text-[10px] font-bold font-mono text-slate-200">BACKEND CLOUD SERVER URL</span>
                    <span className="text-[8px] text-slate-400 uppercase font-mono">
                      Render.com or Cloud WebSocket URL (e.g. https://myraa-ai-backend.onrender.com)
                    </span>
                    <div className="flex gap-2 mt-1">
                      <input
                        type="text"
                        defaultValue={localStorage.getItem("myraa_backend_url") || ""}
                        id="backend-url-input"
                        placeholder="https://myraa-ai-backend.onrender.com"
                        className="flex-1 bg-black/40 border border-white/10 rounded-xl px-3 py-1.5 text-xs font-mono text-cyan-200 outline-none focus:border-cyan-500"
                      />
                      <button
                        onClick={() => {
                          const val = (document.getElementById("backend-url-input") as HTMLInputElement)?.value.trim();
                          if (val) {
                            localStorage.setItem("myraa_backend_url", val);
                          } else {
                            localStorage.removeItem("myraa_backend_url");
                          }
                          alert("Backend URL saved! Reconnect or reload app to apply.");
                        }}
                        className="px-3 py-1.5 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 rounded-xl text-[10px] font-mono font-bold border border-cyan-500/30 transition cursor-pointer"
                      >
                        SAVE URL
                      </button>
                    </div>
