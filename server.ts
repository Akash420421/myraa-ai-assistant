...
const MOBILE_TOOLS: ReadonlySet<string> = new Set([
  "getMobileDeviceInfo",
  "toggleTorch",
  "capturePhotoVision",
...
async function startServer() {
  ...
  // -------------------------------------------------------------------------
  // Keep-alive ping endpoint (Render.com free tier & Uptime monitoring)
  // -------------------------------------------------------------------------
  app.get(["/health", "/api/health"], (_req, res) => {
    res.json({
      status: "ok",
      service: "myraa-ai-backend",
  ...
  wss.on("connection", async (clientWs) => {
    ...
    const pendingClientToolCalls = new Map<string, (res: any) => void>();
    ...
    try {
          ...
          tools: [
            {
              functionDeclarations: [
                ...
                // ======== MOBILE NATIVE TOOLS (routed to Android/Capacitor client) ========
                {
                  name: "getMobileDeviceInfo",
                  description: "Get battery level, charging status, operating system, and hardware model from the user's mobile device.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
                {
                  name: "toggleTorch",
                  description: "Turn the mobile flashlight / torch ON or OFF.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      enable: { type: Type.BOOLEAN, description: "True to turn on, False to turn off (leave empty to toggle)." }
                    }
                  }
                },
                {
                  name: "capturePhotoVision",
                  description: "Capture a live photo using the mobile camera and analyze it visually to answer the user's question about an object, document, or environment.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      prompt: { type: Type.STRING, description: "What to focus on or analyze in the captured photo." }
                    }
                  }
                },
                {
                  name: "scheduleNotification",
                  description: "Schedule or immediately send a local notification alert or alarm to the user's mobile notification tray.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      title: { type: Type.STRING, description: "Title of the alert." },
                      body: { type: Type.STRING, description: "Message content." },
                      delay_seconds: { type: Type.INTEGER, description: "Seconds to wait before sending (0 for instant)." }
                    },
                    required: ["title", "body"]
                  }
                },
                {
                  name: "openWhatsApp",
                  description: "Open WhatsApp to send a pre-filled message or chat with a contact.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      message: { type: Type.STRING, description: "Message text to pre-fill." },
                      phone: { type: Type.STRING, description: "Optional phone number with country code (e.g. +919876543210)." }
                    },
                    required: ["message"]
                  }
                },
                {
                  name: "dialPhone",
                  description: "Open the phone dialer with a specified phone number ready to call.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      phone: { type: Type.STRING, description: "Phone number to dial." }
                    },
                    required: ["phone"]
                  }
                },
                {
                  name: "openMaps",
                  description: "Open Google Maps navigation or search for a location, shop, or directions on the mobile device.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      query: { type: Type.STRING, description: "Search query, destination, or place name." }
                    },
                    required: ["query"]
                  }
                },
                {
                  name: "triggerHaptic",
                  description: "Trigger physical haptic vibration feedback on the mobile device.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      type: { type: Type.STRING, description: "Haptic pattern: 'light', 'medium', 'heavy', 'success', 'warning', 'error'" }
                    }
                  }
                },
                {
                  name: "setMobileClipboard",
                  description: "Copy text or link directly to the mobile device clipboard.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      text: { type: Type.STRING, description: "Text to copy." }
                    },
                    required: ["text"]
                  }
                },
                {
                  name: "getMobileClipboard",
                  description: "Read text from the mobile device clipboard.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
          ...
          onmessage: (message: LiveServerMessage) => {
            ...
            if (message.toolCall?.functionCalls) {
              for (const fc of message.toolCall.functionCalls) {
                ...
                } else if (MOBILE_TOOLS.has(fc.name)) {
                  // Native Mobile Tool execution dispatched to the connected mobile app
                  (async () => {
                    try {
                      console.log(`[Mobile Tool]: ${fc.name} keys=[${Object.keys((fc.args || {}) as object).join(",")}]`);
                      ...
                      const result = await new Promise<any>((resolve) => {
                        const timeout = setTimeout(() => {
                          ...
                          resolve({ ok: false, error: "Mobile action timed out." });
                        }, 20_000);
                        timeout.unref?.();

                        pendingClientToolCalls.set(callId, (res) => {
                          clearTimeout(timeout);
                          resolve(res);
                        });
                      });

                      if (fc.name === "capturePhotoVision" && result?.imageBase64) {
                        session.sendClientContent({
                          turns: [{
                            role: "user",
                            parts: [
                              { text: `${(fc.args as any)?.prompt || "Analyze this live camera photo and answer the user's question directly."}` },
                              { inlineData: { data: result.imageBase64, mimeType: result.mimeType || "image/jpeg" } },
                            ],
                          }],
                          turnComplete: true,
                        });
                      ...
                      console.error(`Mobile tool ${fc.name} execution error:`, err);
                      session.sendToolResponse({
                        functionResponses: [
                          {
                            name: fc.name,
                            response: { output: { ok: false, error: err?.message || String(err) } },
                            id: fc.id,
                          },
                        ],
                      });
      ...
      clientWs.on("message", async (rawMsg) => {
        try {
          ...
          if (msg.type === "client_tool_response" && msg.id) {
            const resolve = pendingClientToolCalls.get(msg.id);
            if (resolve) {
